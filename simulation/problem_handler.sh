...
# problems
PROBLEMS_LIST=('SLURM_NODE_FAIL' 'SLURM_CANCELLED' 'SLURM_FAILED' 'SLURM_TIMEOUT')
...
for idx in `seq 0 4`;  # loop over run periods
do
	echo " --------------------------------------- "
	...
	do
		...
		do
			...
			if [ "${PROBLEMS_LIST[problem_idx]}" == "SLURM_TIMEOUT" ]; then
				if grep -q "cmu.edu" <<< "$LOC_HOSTNAME"; then
					echo swif2 modify-jobs -time add 2h -workflow ${WORKFLOWNAME} -problems ${PROBLEMS_LIST[problem_idx]}
				elif grep -q "jlab.org" <<< "$LOC_HOSTNAME"; then
					swif2 modify-jobs -time add 2h -workflow ${WORKFLOWNAME} -problems ${PROBLEMS_LIST[problem_idx]}
				fi
			else
				if grep -q "cmu.edu" <<< "$LOC_HOSTNAME"; then
					echo swif2 retry-jobs -workflow ${WORKFLOWNAME} -problems ${PROBLEMS_LIST[problem_idx]}
				elif grep -q "jlab.org" <<< "$LOC_HOSTNAME"; then
					swif2 retry-jobs -workflow ${WORKFLOWNAME} -problems ${PROBLEMS_LIST[problem_idx]}
				fi
			fi
